...
2. Open **Windows PowerShell** and run as Administrator.
	...
	- Select **Start** and search for **Windows PowerShell (Admin)**, then right select **Run as administrator**. Note: You maybe prompted to input the password for 
          **Client 1VM**
...
8. Load the Entra ID (Azure AD) unified group template, by using the following cmdlet:

       ```powershell
        $Template = Get-AzureADDirectorySettingTemplate | Where {$_.DisplayName -eq "Group.Unified"}
	    ```

9. Check if an Entra ID (Azure AD) setting is already existing and load it, if yes. If not, create a blank Entra ID (Azure AD) setting object. Run the following cmdlet to populate the "$Setting" variable:
...
17. Load the Entra ID (Azure AD) unified group template, by using the following cmdlet:

        ```powershell
        $Template = Get-AzureADDirectorySettingTemplate | Where {$_.DisplayName -eq "Group.Unified"}
        ```

18.	Create a blank Entra ID (Azure AD) tenant settings object:
