...
7. Load the Entra ID (Azure AD) unified group template, by using the following cmdlet:

       ```powershell
        $Template = Get-AzureADDirectorySettingTemplate | Where {$_.DisplayName -eq "Group.Unified"}
	    ```

8. Check if an Entra ID (Azure AD) setting is already existing and load it, if yes. If not, create a blank Entra ID (Azure AD) setting object. Run the following cmdlet to populate the "$Setting" variable:
...
16. Load the Entra ID (Azure AD) unified group template, by using the following cmdlet:

        ```powershell
        $Template = Get-AzureADDirectorySettingTemplate | Where {$_.DisplayName -eq "Group.Unified"}
        ```

17.	Create a blank Entra ID (Azure AD) tenant settings object:
